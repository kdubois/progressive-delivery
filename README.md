...
Before deploying the stack, configure the Kubernetes agent secrets. The secret is automatically generated by Kustomize from a `secrets.env` file.

#### Option 1: Generate from Environment Variables (Recommended)

If you already have credentials in your environment:
...
**Note:** The `secrets.env` file is git-ignored for security and located in the project root. Kustomize will automatically generate the Kubernetes secret from this file during deployment.

The `secrets.env` file expects these values:
